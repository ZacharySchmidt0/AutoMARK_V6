classdef criterionCentermarkPosition < baseCriterion
    % Criterion which does X
    
    methods
        function obj = criterionCentermarkPosition(recommendedweight)
            ...
            obj.tolerancetip = "Rectangles are a set of rectangles given as boxes around the original centermark, units are meters and you can add more rectangles by adding additional rows of 4 numbers as xmin, ymin, xmax, ymax.";
        ...
        function multiplier = evaluateOn(obj, linker, keyFeature, studentReport, relevantSheetHandler, doAnnotations,criterionColours)
            ...
            if doAnnotations
                if multiplier == 0
                    studentReport.addComment(sprintf("Centermark ""%s"" was in a suitable location", studentFeature.name));
                    
                else
                    studentReport.addComment(sprintf("Centermark ""%s"" wasn't in a suitable location", studentFeature.name));
                    % Arrow from student to key, missing coloured
                    % This is a bit of an oddball criterion anyway.
