classdef criterionDimensionPostion < baseCriterion
    % Criterion which does X
    
    methods
        function obj = criterionDimensionPostion(recommendedweight)
            ...
            obj.tolerancetip = "Rectangles are a set of rectangles given as boxes around the original dimension, units are meters and you can add more rectangles by adding additional rows of 4 numbers as xmin, ymin, xmax, ymax.";
        ...
        function multiplier = evaluateOn(obj, linker, keyFeature, studentReport, relevantSheetHandler, doAnnotations,criterionColours)
            ...
            if doAnnotations
                if multiplier == 0
                    studentReport.addComment(sprintf("Dimension ""%s"" was in a suitable location", studentFeature.name));
                else
                    studentReport.addComment(sprintf("Dimension ""%s"" wasn't in a suitable location", studentFeature.name));

                    % Arrow from student to key, missing coloured
                    
                    % If the dimension isn't cross linked, then do this,
